...
func (t *SimpleChaincode) LogEvent(ctx contractapi.TransactionContextInterface, deviceId string, value int, measurementUnit string) error {
	...
	err = ctx.GetStub().PutState(uuid.New().String(), assetBytes)
	if err != nil {
		return err
	}
	return nil
...
func (t *SimpleChaincode) InitLedger(ctx contractapi.TransactionContextInterface) error {
	logs := []RequestLog{
		{DocType: "log", DeviceId: "asdf", Value: 600, MeasurementUnit: "HUMD"},
		{DocType: "log", DeviceId: "asdf", Value: 610, MeasurementUnit: "HUMD"},
		{DocType: "log", DeviceId: "asdf", Value: 620, MeasurementUnit: "HUMD"},
		{DocType: "log", DeviceId: "asdf", Value: 630, MeasurementUnit: "HUMD"},
	}

	for _, log := range logs {
		err := t.LogEvent(ctx, log.DeviceId, log.Value, log.MeasurementUnit)
		if err != nil {
			return err
		}
	}
